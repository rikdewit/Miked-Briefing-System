...
export const GlobalChat: React.FC<GlobalChatProps> = ({
  ...
  if (selectedItem) {
    ...
    const handleSubmitComment = (e: React.FormEvent) => {
      e.preventDefault();
      if (text.trim()) { onAddComment(item.id, text.trim()); setText(''); }
    };
          ...
          <div className="px-6 pt-5 pb-4 space-y-5 shrink-0">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-4 font-mono text-sm mt-3">
                ...
                {item.specs?.model && (<><div className="col-span-1 opacity-60">Model</div><div className="col-span-2">{item.specs.model}</div></>)}
                {item.specs?.quantity && (<><div className="col-span-1 opacity-60">Quantity</div><div className="col-span-2">{item.specs.quantity}</div></>)}
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            ...
            <div className="space-y-4 overflow-y-auto flex-1 px-6 pt-4 pb-4">
              ...
              {item.comments.map((comment) => {
                        ...
                        <div className="space-y-3 opacity-70">
                          ...
                          {comment.previousData?.provider && comment.newData?.provider && (
                            <div className="flex flex-col gap-1">
                              ...
                              <div className="flex items-center gap-2">
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold opacity-50 line-through ${comment.previousData.provider === 'BAND' ? 'bg-indigo-100 text-indigo-800' : comment.previousData.provider === 'VENUE' ? 'bg-neutral-100 text-neutral-800' : 'bg-cyan-100 text-cyan-800'}`}>{comment.previousData.provider}</span>
                                <span className="text-[10px] opacity-50">→</span>
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${comment.newData.provider === 'BAND' ? 'bg-indigo-100 text-indigo-800' : comment.newData.provider === 'VENUE' ? 'bg-neutral-100 text-neutral-800' : 'bg-cyan-100 text-cyan-800'}`}>{comment.newData.provider}</span>
                              </div>
                            </div>
                          )}
                          ...
                          {comment.previousData?.specs && comment.newData?.specs && (
                            <div className="flex flex-col gap-1">
                              ...
                              <div className="grid grid-cols-2 gap-x-4 gap-y-1 pl-2 opacity-50 line-through text-[10px]">
                                {Object.entries(comment.previousData.specs).map(([k, v]) => <div key={k}><span className="capitalize">{k}:</span> {String(v)}</div>)}
                              </div>
                              <div className="grid grid-cols-2 gap-x-4 gap-y-1 pl-2 text-emerald-700 text-[10px]">
                                {Object.entries(comment.newData.specs).map(([k, v]) => <div key={k}><span className="capitalize">{k}:</span> {String(v)}</div>)}
                              </div>
                            </div>
                          )}
                        </div>
                ...
                const bubbleStyle = comment.role === 'BAND' ? 'bg-indigo-100 text-indigo-900 border-indigo-300' : 'bg-cyan-100 text-cyan-900 border-cyan-300';
                return (
                  <div key={comment.id} className={`flex flex-col ${comment.role === role ? 'items-end' : 'items-start'}`}>
                    <div className={`max-w-[85%] p-3 border ${bubbleStyle}`}><p className="text-sm">{comment.text}</p></div>
                    <span className="font-mono text-[10px] mt-1 opacity-50">
